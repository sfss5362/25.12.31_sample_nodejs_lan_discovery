...
```json
{
  ...
  "knownDevices": [
    "192.168.1.24"  // 已知设备 IP（秒级快速发现）⚡
  ]
}
```

**已知设备快速扫描**：
- 添加常用设备 IP 到 `knownDevices` 数组
- 扫描时优先检测这些设备（<1 秒）
- 然后才进行完整子网扫描
- 适合办公室固定设备（同事电脑、打印机等）
...
- **快速扫描优化**: HTTP 超时 500ms，并发 100 个请求，比默认配置快 30-50%
- **已知设备加速**: 配置常用设备 IP，秒级快速发现（<1 秒）
- **防自响应**: fingerprint 过滤
- **设备超时**: 10秒未响应自动移除
- **端口冲突**: 自动递增 (53318, 53319...)
- **并发扫描**: 100 个并发请求（优化后）
- **快速超时**: HTTP 请求 500ms 超时（优化后）
